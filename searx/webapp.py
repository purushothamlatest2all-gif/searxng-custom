...
def api_spelling():
    ...
    try:
        ...
        logger.error(f"Spelling API error: {e}")
...
def api_overview():
    ...
    try:
        from searx.ai_overview import generate_overview
        
        # Get query and search results from request
        if sxng_request.method == 'POST':
            data = sxng_request.get_json()
            query = data.get('query', '')
            search_results = data.get('search_results', [])
        else:
            query = sxng_request.args.get('q', '')
            search_results = []
        
        if not query:
            return jsonify({'error': 'No query provided'}), 400
        
        # Generate overview with search results
        result = generate_overview(query, search_results)
        
        if result:
            return jsonify(result)
        else:
            return jsonify({'overview': None, 'message': 'No overview available'}), 200
